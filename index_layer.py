...
def _resolve_one(key: str) -> tuple["IndexInstrument | None", str]:
    symbol, aliases = INDEX_SPECS[key]
    ...
    try:
        ...
        for row in rows:
            ...
            if instrument != INDEX_INSTRUMENT:
                continue
            ...
            if {_norm(v) for v in values} & wanted:
                ...
                matches[(security_id, exchange, instrument)] = IndexInstrument(
                    security_id, exchange or "NSE", instrument
                )
        if len(matches) == 1:
