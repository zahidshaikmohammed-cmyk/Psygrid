...
from niftyinfra import NiftyInfraManager, niftyinfra_json
...
settings = state = manager = nifty_manager = banknifty_manager = sensex_manager = nifty_options_manager = nifty_depth_manager = banknifty_options_manager = banknifty_depth_manager = midcpnifty_options_manager = midcpnifty_depth_manager = None
nifty500_manager = niftymidcap100_manager = niftysmallcap100_manager = finnifty_manager = None
indiavix_manager = niftyit_manager = niftyauto_manager = niftypharma_manager = niftymetal_manager = niftyfmcg_manager = niftyrealty_manager = niftyenergy_manager = niftyinfra_manager = None
config_error = ""

def startup() -> None:
    global settings, state, manager, nifty_manager, banknifty_manager, sensex_manager, nifty_options_manager, nifty_depth_manager, banknifty_options_manager, banknifty_depth_manager, midcpnifty_options_manager, midcpnifty_depth_manager, nifty500_manager, niftymidcap100_manager, niftysmallcap100_manager, finnifty_manager, indiavix_manager, niftyit_manager, niftyauto_manager, niftypharma_manager, niftymetal_manager, niftyfmcg_manager, niftyrealty_manager, niftyenergy_manager, niftyinfra_manager, config_error
    config_error = ""
    try:
        settings = load_settings(); instruments = load_instruments()
        if len(instruments) != settings.max_instruments: raise RuntimeError(f"Universe integrity failure: expected {settings.max_instruments}, got {len(instruments)}")
        ...
        managers = [("nifty_manager", NiftyManager), ("banknifty_manager", BankNiftyManager), ("sensex_manager", SensexManager), ("nifty500_manager", Nifty500Manager), ("niftymidcap100_manager", NiftyMidcap100Manager), ("niftysmallcap100_manager", NiftySmallcap100Manager), ("finnifty_manager", FinNiftyManager), ("indiavix_manager", IndiaVixManager), ("niftyit_manager", NiftyItManager), ("niftyauto_manager", NiftyAutoManager), ("niftypharma_manager", NiftyPharmaManager), ("niftymetal_manager", NiftyMetalManager), ("niftyfmcg_manager", NiftyFmcgManager), ("niftyrealty_manager", NiftyRealtyManager), ("niftyenergy_manager", NiftyEnergyManager), ("niftyinfra_manager", NiftyInfraManager)]
        for name, cls in managers:
            try:
                obj = cls(settings, dhan_api); obj.start(); globals()[name] = obj
            except Exception: globals()[name] = None
        try: nifty_options_manager = NiftyOptionsManager(settings, dhan_api); nifty_options_manager.start()
        except Exception: nifty_options_manager = None
        if nifty_options_manager is not None:
            try: nifty_depth_manager = NiftyDepthManager(settings, dhan_api, nifty_options_manager); nifty_depth_manager.start()
            except Exception: nifty_depth_manager = None
        try: banknifty_options_manager = BankNiftyOptionsManager(settings, dhan_api); banknifty_options_manager.start()
        except Exception: banknifty_options_manager = None
        if banknifty_options_manager is not None:
            try: banknifty_depth_manager = BankNiftyDepthManager(settings, dhan_api, banknifty_options_manager); banknifty_depth_manager.start()
            except Exception: banknifty_depth_manager = None
        try: midcpnifty_options_manager = MidcapNiftyOptionsManager(settings, dhan_api); midcpnifty_options_manager.start()
        except Exception: midcpnifty_options_manager = None
        if midcpnifty_options_manager is not None:
            try: midcpnifty_depth_manager = MidcapNiftyDepthManager(settings, dhan_api, midcpnifty_options_manager); midcpnifty_depth_manager.start()
            except Exception: midcpnifty_depth_manager = None
    except Exception as exc: config_error = str(exc)

def shutdown() -> None:
    global manager, nifty_manager, banknifty_manager, sensex_manager, nifty_options_manager, nifty_depth_manager, banknifty_options_manager, banknifty_depth_manager, midcpnifty_options_manager, midcpnifty_depth_manager, nifty500_manager, niftymidcap100_manager, niftysmallcap100_manager, finnifty_manager, indiavix_manager, niftyit_manager, niftyauto_manager, niftypharma_manager, niftymetal_manager, niftyfmcg_manager, niftyrealty_manager, niftyenergy_manager, niftyinfra_manager
    for name in ("midcpnifty_depth_manager","midcpnifty_options_manager","banknifty_depth_manager","banknifty_options_manager","nifty_depth_manager","nifty_options_manager","niftyinfra_manager","niftyenergy_manager","niftyrealty_manager","niftyfmcg_manager","niftymetal_manager","niftypharma_manager","niftyauto_manager","niftyit_manager","indiavix_manager","finnifty_manager","niftysmallcap100_manager","niftymidcap100_manager","nifty500_manager","sensex_manager","banknifty_manager","nifty_manager","manager"):
        obj = globals().get(name)
        if obj is not None: obj.stop(); globals()[name] = None

@asynccontextmanager
async def lifespan(_app: FastAPI): startup(); yield; shutdown()
...
for route,start,end,preserve in [("a",0,45,False),("b",45,90,False),("c",90,135,False),("d",135,180,False),("e",180,225,False),("f",225,270,False),("g",270,315,True),("h",315,360,True),("i",360,405,True),("j",405,450,True),("01",0,15,False),("02",15,30,False),("03",30,45,False),("04",45,60,False),("05",60,75,False),("06",75,90,False)]: globals()[f"public_live_{route}"]=app.get(f"/public/live-{route}.json",response_class=Response)(lambda start=start,end=end,preserve=preserve:_public_live_range(start,end,preserve))
...
@app.get("/public/niftyinfra.json",response_class=Response)
def public_niftyinfra()->Response:return _index_response(niftyinfra_manager,"NIFTY_INFRA",niftyinfra_json)

@app.get("/public/nifty-options.json",response_class=Response)
def public_nifty_options()->Response:
    error=_error_response()
    if error:return error
    if nifty_options_manager is None:return json_response({"service":"PSYGRID","symbol":"NIFTY","status":"NIFTY_OPTIONS_UNAVAILABLE"},503)
    payload=nifty_options_json(nifty_options_manager.state); return json_response(payload,200 if payload.get("status")=="LIVE" else 503)
@app.get("/public/nifty-depth.json",response_class=Response)
def public_nifty_depth()->Response:
    error=_error_response()
    if error:return error
    if nifty_depth_manager is None:return json_response({"service":"PSYGRID","symbol":"NIFTY","status":"NIFTY_DEPTH_UNAVAILABLE"},503)
    payload=nifty_depth_json(nifty_depth_manager.state); return json_response(payload,200 if payload.get("status")=="LIVE" else 503)
@app.get("/public/banknifty-options.json",response_class=Response)
def public_banknifty_options()->Response:
    error=_error_response()
    if error:return error
    if banknifty_options_manager is None:return json_response({"service":"PSYGRID","symbol":"BANKNIFTY","status":"BANKNIFTY_OPTIONS_UNAVAILABLE"},503)
    payload=banknifty_options_json(banknifty_options_manager.state); return json_response(payload,200 if payload.get("status")=="LIVE" else 503)
@app.get("/public/banknifty-depth.json",response_class=Response)
def public_banknifty_depth()->Response:
    error=_error_response()
    if error:return error
    if banknifty_depth_manager is None:return json_response({"service":"PSYGRID","symbol":"BANKNIFTY","status":"BANKNIFTY_DEPTH_UNAVAILABLE"},503)
    payload=banknifty_depth_json(banknifty_depth_manager.state); return json_response(payload,200 if payload.get("status")=="LIVE" else 503)
@app.get("/public/midcpnifty-options.json",response_class=Response)
def public_midcpnifty_options()->Response:
    error=_error_response()
    if error:return error
    if midcpnifty_options_manager is None:return json_response({"service":"PSYGRID","symbol":"MIDCPNIFTY","status":"MIDCPNIFTY_OPTIONS_UNAVAILABLE"},503)
    payload=midcpnifty_options_json(midcpnifty_options_manager.state); return json_response(payload,200 if payload.get("status")=="LIVE" else 503)
@app.get("/public/midcpnifty-depth.json",response_class=Response)
def public_midcpnifty_depth()->Response:
    error=_error_response()
    if error:return error
    if midcpnifty_depth_manager is None:return json_response({"service":"PSYGRID","symbol":"MIDCPNIFTY","status":"MIDCPNIFTY_DEPTH_UNAVAILABLE"},503)
    payload=midcpnifty_depth_json(midcpnifty_depth_manager.state); return json_response(payload,200 if payload.get("status")=="LIVE" else 503)
...
if __name__=="__main__":uvicorn.run(app,host="0.0.0.0",port=int(os.getenv("PORT","10000")),access_log=False)
