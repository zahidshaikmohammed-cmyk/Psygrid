...
settings = state = manager = nifty_manager = banknifty_manager = sensex_manager = nifty_options_manager = nifty_depth_manager = None
...
def public_nifty_options() -> Response:
    error = _error_response()
    if error: return error
    if nifty_options_manager is None:
        return json_response({"service":"PSYGRID","symbol":"NIFTY","status":"NIFTY_OPTIONS_UNAVAILABLE"},503)
    payload = nifty_options_json(nifty_options_manager.state)
...
def public_nifty_depth() -> Response:
    error = _error_response()
    if error: return error
    if nifty_depth_manager is None:
        return json_response({"service":"PSYGRID","symbol":"NIFTY","status":"NIFTY_DEPTH_UNAVAILABLE"},503)
    payload = nifty_depth_json(nifty_depth_manager.state)
    return json_response(payload, 200 if payload.get("status") == "LIVE" else 503)
