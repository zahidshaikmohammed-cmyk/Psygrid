...
NIFTYIT_SYMBOL = "NIFTY_IT"
NIFTYIT_EXCHANGE_SEGMENT = "IDX_I"
NIFTYIT_INSTRUMENT = "INDEX"
NIFTYIT_MASTER_URL = "https://images.dhan.co/api-data/api-scrip-master.csv"
...
def resolve_niftyit_security_id() -> str:
    response = requests.get(NIFTYIT_MASTER_URL, timeout=15)
    response.raise_for_status()
    reader = csv.DictReader(io.StringIO(response.text))
    candidates = {_norm("NIFTY IT"), _norm("NIFTY_IT"), _norm("NIFTYIT")}
    matches: list[str] = []
    for row in reader:
        ...
        if str(row.get("SEM_SEGMENT", "")).strip().upper() != "I":
            continue
        if str(row.get("SEM_INSTRUMENT_NAME", "")).strip().upper() != NIFTYIT_INSTRUMENT:
            continue
    ...
    if len(unique) != 1:
        raise RuntimeError(f"Dhan instrument master could not uniquely resolve {NIFTYIT_SYMBOL}: {unique}")
    return unique[0]


class NiftyItInstrument(NiftyInstrument):
    def __init__(self, security_id: str):
        object.__setattr__(self, "security_id", str(security_id))
        object.__setattr__(self, "exchange_segment", NIFTYIT_EXCHANGE_SEGMENT)
        object.__setattr__(self, "instrument", NIFTYIT_INSTRUMENT)


class NiftyItState(NiftyState):
    """RAM-only state for the NIFTY IT index endpoint."""


class NiftyItFeed(NiftyFeed):
    def __init__(self, settings, state: NiftyItState, security_id: str):
    ...
    def _on_message(self, _feed, data) -> None:
        ...
        if packet_type == "error":
            self.state.set_feed_status("ERROR", f"Dhan {NIFTYIT_SYMBOL} feed error code={data.get('error_code', data.get('code'))}: {data.get('message', data.get('error_message', 'feed error'))}")
            return
...
class NiftyItManager(NiftyManager):
    def __init__(self, settings, dhan_api):
        self.settings = settings
        self.dhan_api = dhan_api
        security_id = resolve_niftyit_security_id()
        self.state = NiftyItState(settings)
        self.feed = NiftyItFeed(settings, self.state, security_id)
        self.stop_event = threading.Event()
        self.thread: Optional[threading.Thread] = None
        self.htf_threads: dict[str, threading.Thread] = {}
        self.instrument = NiftyItInstrument(security_id)


def niftyit_json(state: NiftyItState, security_id: str) -> dict:
    with state.lock:
