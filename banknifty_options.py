...
def _normalize_chain(raw):
    ...
    if not isinstance(chain, dict): return []
    rows=[]
    for strike_key,pair in chain.items():
        if not isinstance(pair,dict): continue
        try: strike=float(strike_key)
        except (TypeError,ValueError): continue
        ce=pair.get("ce") if isinstance(pair.get("ce"),dict) else None; pe=pair.get("pe") if isinstance(pair.get("pe"),dict) else None
        rows.append({"strike":strike,"ce":dict(ce) if ce else None,"pe":dict(pe) if pe else None})
    rows.sort(key=lambda row:row["strike"]); return rows
...
class BankNiftyOptionsManager:
    ...
    def stop(self):
        self.stop_event.set()
        if self.thread and self.thread is not threading.current_thread(): self.thread.join(timeout=8)
        self.thread=None
    @staticmethod
    def _looks_like_auth_failure(exc):
        text=str(exc).lower(); return any(token in text for token in ("401","807","808","809","810","expired","invalid token","authentication failed","unauthorized"))
    def _call_with_auth_retry(self, operation):
        now=time.monotonic()
        with self._auth_lock:
            if now < self._auth_retry_at: raise RuntimeError(f"Dhan authentication refresh cooldown active: {int(self._auth_retry_at-now)}s")
        try: return operation()
        except Exception as first_exc:
            if not self._looks_like_auth_failure(first_exc): raise
            with self._auth_lock:
                if time.monotonic() < self._auth_retry_at: raise RuntimeError("Dhan authentication refresh cooldown active") from first_exc
                try: refresh_access_token(self.settings, force=True)
                except DhanTokenRateLimited as exc: self._auth_retry_at=time.monotonic()+exc.retry_after; raise
                self.dhan_api.settings=self.settings; self._auth_retry_at=0.0
            return operation()
    def _load_expiries(self):
        expiries=self._call_with_auth_retry(lambda:self.dhan_api.option_expiry_list(self.instrument))
        if not expiries: raise RuntimeError("DHAN_BANKNIFTY_OPTIONS_NO_ACTIVE_EXPIRIES")
        self._expiry_loaded_at=time.monotonic(); return expiries
    def _loop(self):
        expiries=[]; expiry=None
        while not self.stop_event.is_set():
            try:
                if not expiries or time.monotonic()-self._expiry_loaded_at>=EXPIRY_REFRESH_SECONDS: expiries=self._load_expiries(); expiry=expiries[0]
