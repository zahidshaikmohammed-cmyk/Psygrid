...
NIFTY_DEPTH_EXCHANGE_SEGMENT = "NSE_FNO"
NIFTY_DEPTH_INSTRUMENT = "OPTIDX"
NIFTY_DEPTH_SYMBOL = "NIFTY"
NIFTY_DEPTH_SECURITY_ID = "13"
NIFTY_DEPTH_LEVELS = 20
NIFTY_DEPTH_MAX_INSTRUMENTS = 50
NIFTY_DEPTH_RECONNECT_SECONDS = 3.0
NIFTY_DEPTH_QUOTE_REFRESH_SECONDS = 1.0
NIFTY_MARKET_OPEN = datetime_time(9, 15)
NIFTY_MARKET_CLOSE = datetime_time(15, 30)


@dataclass(frozen=True)
class NiftyDepthContract:
    security_id: str
    strike: float
    option_type: str
    expiry: str
...
class NiftyDepthState:
    """RAM-only current NIFTY option premium-depth snapshot."""
    ...
    def set_contracts(self, contracts: list[NiftyDepthContract], expiry: str) -> None:
        with self.lock:
            self.expiry = expiry
    ...
    def snapshot(self) -> dict:
        with self.lock:
            ...
            return {
                "service": "PSYGRID",
                "symbol": NIFTY_DEPTH_SYMBOL,
                "status": self.status,
                "market_status": "OPEN" if market_open else "CLOSED",
                "market_open": market_open,
                "data_source": "DHAN_FULL_MARKET_DEPTH_WEBSOCKET",
                "underlying_security_id": NIFTY_DEPTH_SECURITY_ID,
                "exchange_segment": NIFTY_DEPTH_EXCHANGE_SEGMENT,
                "instrument": NIFTY_DEPTH_INSTRUMENT,
                "depth_levels": NIFTY_DEPTH_LEVELS,
                ...
                "storage": "RAM_ONLY",
                "quote_refresh_seconds": NIFTY_DEPTH_QUOTE_REFRESH_SECONDS,
...
def _select_contracts(option_state) -> tuple[list[NiftyDepthContract], str | None, float | None]:
    ...
    contracts = [NiftyDepthContract(security_id=item[3], strike=item[1], option_type=item[2], expiry=str(expiry)) for item in normalized if item[1] in selected_strikes]
    contracts.sort(key=lambda item: (abs(item.strike - float(underlying_ltp)), item.strike, item.option_type))
    return contracts[:NIFTY_DEPTH_MAX_INSTRUMENTS], str(expiry), float(underlying_ltp)
...
def _parse_depth_message(data: bytes) -> list[tuple[str, str, list[dict]]]:
    ...
    while offset + 12 <= len(data):
        ...
        if response_code in (41, 51) and message_length >= 332:
            ...
            levels = []
            base = offset + 12
            for index in range(NIFTY_DEPTH_LEVELS):
...
class NiftyDepthManager:
    """Maintains real-time 20-level premium depth for the nearest 25 NIFTY strikes."""

    def __init__(self, settings, dhan_api, option_manager):
        self.settings = settings
        self.dhan_api = dhan_api
        self.option_manager = option_manager
        self.state = NiftyDepthState(settings)
        self.stop_event = threading.Event()
        self.thread: threading.Thread | None = None
        self.ws = None
        self._contracts: list[NiftyDepthContract] = []
    ...
    def start(self) -> None:
        ...
        self.thread = threading.Thread(target=self._loop, daemon=True, name="psygrid-nifty-depth")
        self.thread.start()
    ...
    def _subscribe_payload(self) -> str:
        instruments = [{"ExchangeSegment": NIFTY_DEPTH_EXCHANGE_SEGMENT, "SecurityId": contract.security_id} for contract in self._contracts]
        return __import__("json").dumps({"RequestCode": 23, "InstrumentCount": len(instruments), "InstrumentList": instruments})
    ...
    def _quote_loop(self) -> None:
        while not self.stop_event.is_set():
            try:
                contracts, expiry, underlying_ltp = _select_contracts(self.option_manager.state)
                if contracts:
                    quotes = self.dhan_api.quote_snapshot([type("DepthInstrument", (), {"security_id": c.security_id, "exchange_segment": NIFTY_DEPTH_EXCHANGE_SEGMENT})() for c in contracts])
            ...
            self.stop_event.wait(NIFTY_DEPTH_QUOTE_REFRESH_SECONDS)

    def _loop(self) -> None:
        quote_thread = threading.Thread(target=self._quote_loop, daemon=True, name="psygrid-nifty-depth-quotes")
        quote_thread.start()
        while not self.stop_event.is_set():
            try:
                ...
                self.state.set_error(f"{type(exc).__name__}: {exc}")
                self.stop_event.wait(NIFTY_DEPTH_RECONNECT_SECONDS)
        quote_thread.join(timeout=2)


def nifty_depth_json(state: NiftyDepthState) -> dict:
