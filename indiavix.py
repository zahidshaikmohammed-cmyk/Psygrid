...
INDIAVIX_SYMBOL = "INDIA VIX"
INDIAVIX_EXCHANGE_SEGMENT = "IDX_I"
INDIAVIX_INSTRUMENT = "INDEX"
INDIAVIX_MASTER_URL = "https://images.dhan.co/api-data/api-scrip-master.csv"
...
def resolve_indiavix_security_id() -> str:
    """Resolve the current India VIX security ID from Dhan's public instrument master."""
    response = requests.get(INDIAVIX_MASTER_URL, timeout=15)
    ...
    for row in reader:
        ...
        if exchange != "NSE" or segment != "I" or instrument != INDIAVIX_INSTRUMENT:
            continue
        values = (
    ...
    if len(unique) != 1:
        raise RuntimeError(f"Dhan instrument master could not uniquely resolve {INDIAVIX_SYMBOL}: {unique}")
    return unique[0]


class IndiaVixInstrument(NiftyInstrument):
    def __init__(self, security_id: str):
        object.__setattr__(self, "security_id", str(security_id))
        object.__setattr__(self, "exchange_segment", INDIAVIX_EXCHANGE_SEGMENT)
        object.__setattr__(self, "instrument", INDIAVIX_INSTRUMENT)


class IndiaVixState(NiftyState):
    """RAM-only state for the India VIX index endpoint."""


class IndiaVixFeed(NiftyFeed):
    def __init__(self, settings, state: IndiaVixState, security_id: str):
    ...
    def _on_message(self, _feed, data) -> None:
        ...
        if packet_type == "error":
            self.state.set_feed_status("ERROR", f"Dhan {INDIAVIX_SYMBOL} feed error code={data.get('error_code', data.get('code'))}: {data.get('message', data.get('error_message', 'feed error'))}")
            return
...
class IndiaVixManager(NiftyManager):
    def __init__(self, settings, dhan_api):
        self.settings = settings
        self.dhan_api = dhan_api
        security_id = resolve_indiavix_security_id()
        self.state = IndiaVixState(settings)
        self.feed = IndiaVixFeed(settings, self.state, security_id)
        self.stop_event = threading.Event()
        self.thread: Optional[threading.Thread] = None
        self.htf_threads: dict[str, threading.Thread] = {}
        self.instrument = IndiaVixInstrument(security_id)


def indiavix_json(state: IndiaVixState, security_id: str) -> dict:
    with state.lock:
    ...
    return {
        "service": "PSYGRID",
        "schema_version": "3.0",
        "symbol": INDIAVIX_SYMBOL,
        "security_id": str(security_id),
        "exchange_segment": INDIAVIX_EXCHANGE_SEGMENT,
        "instrument": INDIAVIX_INSTRUMENT,
