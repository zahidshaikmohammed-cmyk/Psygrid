...
NIFTY500_SYMBOL = "NIFTY500"
NIFTY500_SECURITY_ID = "17"
NIFTY500_EXCHANGE_SEGMENT = "IDX_I"
NIFTY500_INSTRUMENT = "INDEX"


class Nifty500Instrument(NiftyInstrument):
    def __init__(self):
        object.__setattr__(self, "security_id", NIFTY500_SECURITY_ID)
        object.__setattr__(self, "exchange_segment", NIFTY500_EXCHANGE_SEGMENT)
        object.__setattr__(self, "instrument", NIFTY500_INSTRUMENT)


class Nifty500State(NiftyState):
    """RAM-only state for the NIFTY 500 index endpoint."""


class Nifty500Feed(NiftyFeed):
    def _build_feed(self):
        context = DhanContext(self.settings.client_id, self.settings.access_token)
        return MarketFeed(
            context,
            [(MarketFeed.IDX, NIFTY500_SECURITY_ID, MarketFeed.Full)],
            version="v2",
    ...
    def _on_message(self, _feed, data) -> None:
        ...
        if packet_type == "error":
            self.state.set_feed_status(
                "ERROR",
                f"Dhan NIFTY500 feed error code={data.get('error_code', data.get('code'))}: "
        ...
        try:
            ...
            if security_id != NIFTY500_SECURITY_ID:
                return
...
class Nifty500Manager(NiftyManager):
    def __init__(self, settings, dhan_api):
        self.settings = settings
        self.dhan_api = dhan_api
        self.state = Nifty500State(settings)
        self.feed = Nifty500Feed(settings, self.state)
        self.stop_event = threading.Event()
        self.thread: Optional[threading.Thread] = None
        self.htf_threads: dict[str, threading.Thread] = {}
        self.instrument = Nifty500Instrument()


def nifty500_json(state: Nifty500State) -> dict:
    with state.lock:
    ...
    return {
        "service": "PSYGRID",
        "schema_version": "3.0",
        "symbol": NIFTY500_SYMBOL,
        "security_id": NIFTY500_SECURITY_ID,
        "exchange_segment": NIFTY500_EXCHANGE_SEGMENT,
        "instrument": NIFTY500_INSTRUMENT,
