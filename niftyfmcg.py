...
NIFTYFMCG_SYMBOL = "NIFTY_FMCG"
NIFTYFMCG_EXCHANGE_SEGMENT = "IDX_I"
NIFTYFMCG_INSTRUMENT = "INDEX"
NIFTYFMCG_MASTER_URL = "https://images.dhan.co/api-data/api-scrip-master.csv"
...
def resolve_niftyfmcg_security_id() -> str:
    response = requests.get(NIFTYFMCG_MASTER_URL, timeout=15)
    response.raise_for_status()
    reader = csv.DictReader(io.StringIO(response.text))
    candidates = {_norm("NIFTY FMCG"), _norm("NIFTY_FMCG"), _norm("NIFTYFMCG")}
    matches: list[str] = []
    for row in reader:
        ...
        if str(row.get("SEM_SEGMENT", "")).strip().upper() != "I":
            continue
        if str(row.get("SEM_INSTRUMENT_NAME", "")).strip().upper() != NIFTYFMCG_INSTRUMENT:
            continue
    ...
    if len(unique) != 1:
        raise RuntimeError(f"Dhan instrument master could not uniquely resolve {NIFTYFMCG_SYMBOL}: {unique}")
    return unique[0]


class NiftyFmcgInstrument(NiftyInstrument):
    def __init__(self, security_id: str):
        object.__setattr__(self, "security_id", str(security_id))
        object.__setattr__(self, "exchange_segment", NIFTYFMCG_EXCHANGE_SEGMENT)
        object.__setattr__(self, "instrument", NIFTYFMCG_INSTRUMENT)


class NiftyFmcgState(NiftyState):
    """RAM-only state for the NIFTY FMCG index endpoint."""


class NiftyFmcgFeed(NiftyFeed):
    def __init__(self, settings, state: NiftyFmcgState, security_id: str):
    ...
    def _on_message(self, _feed, data) -> None:
        ...
        if packet_type == "error":
            self.state.set_feed_status("ERROR", f"Dhan {NIFTYFMCG_SYMBOL} feed error code={data.get('error_code', data.get('code'))}: {data.get('message', data.get('error_message', 'feed error'))}")
            return
...
class NiftyFmcgManager(NiftyManager):
    def __init__(self, settings, dhan_api):
        self.settings = settings
        self.dhan_api = dhan_api
        security_id = resolve_niftyfmcg_security_id()
        self.state = NiftyFmcgState(settings)
        self.feed = NiftyFmcgFeed(settings, self.state, security_id)
        self.stop_event = threading.Event()
        self.thread: Optional[threading.Thread] = None
        self.htf_threads: dict[str, threading.Thread] = {}
        self.instrument = NiftyFmcgInstrument(security_id)


def niftyfmcg_json(state: NiftyFmcgState, security_id: str) -> dict:
    with state.lock:
