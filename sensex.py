...
SENSEX_INSTRUMENT = "INDEX"


class SensexInstrument(NiftyInstrument):
    security_id: str = SENSEX_SECURITY_ID
    exchange_segment: str = SENSEX_EXCHANGE_SEGMENT
    instrument: str = SENSEX_INSTRUMENT

    def __init__(self):
        object.__setattr__(self, "security_id", SENSEX_SECURITY_ID)
        object.__setattr__(self, "exchange_segment", SENSEX_EXCHANGE_SEGMENT)
        object.__setattr__(self, "instrument", SENSEX_INSTRUMENT)


class SensexState(NiftyState):
    """RAM-only state for the SENSEX index endpoint."""


class SensexFeed(NiftyFeed):
    def _build_feed(self):
        context = DhanContext(self.settings.client_id, self.settings.access_token)
        return MarketFeed(
            context,
            [(MarketFeed.IDX, SENSEX_SECURITY_ID, MarketFeed.Full)],
            version="v2",
    ...
    def _on_message(self, _feed, data) -> None:
        ...
        if packet_type == "error":
            self.state.set_feed_status(
                "ERROR",
                f"Dhan SENSEX feed error code={data.get('error_code', data.get('code'))}: "
...
class SensexManager(NiftyManager):
    def __init__(self, settings, dhan_api):
        self.settings = settings
        self.dhan_api = dhan_api
        self.state = SensexState(settings)
        self.feed = SensexFeed(settings, self.state)
        self.stop_event = threading.Event()
        self.thread: Optional[threading.Thread] = None
        self.htf_threads: dict[str, threading.Thread] = {}
        self.instrument = SensexInstrument()


def sensex_json(state: SensexState) -> dict:
    with state.lock:
