...
BANKNIFTY_DEPTH_EXCHANGE_SEGMENT = "NSE_FNO"
BANKNIFTY_DEPTH_INSTRUMENT = "OPTIDX"
BANKNIFTY_DEPTH_SYMBOL = "BANKNIFTY"
BANKNIFTY_DEPTH_SECURITY_ID = "25"
BANKNIFTY_DEPTH_LEVELS = 20
BANKNIFTY_DEPTH_MAX_INSTRUMENTS = 50
BANKNIFTY_DEPTH_RECONNECT_SECONDS = 3.0
BANKNIFTY_DEPTH_QUOTE_REFRESH_SECONDS = 1.0
BANKNIFTY_MARKET_OPEN = datetime_time(9, 15)
BANKNIFTY_MARKET_CLOSE = datetime_time(15, 30)


@dataclass(frozen=True)
class BankNiftyDepthContract:
    security_id: str
    strike: float
    option_type: str
    expiry: str
...
class BankNiftyDepthState:
    """RAM-only current BANKNIFTY option premium-depth snapshot."""
    ...
    def set_contracts(self, contracts: list[BankNiftyDepthContract], expiry: str) -> None:
        with self.lock:
            self.expiry = expiry
            allowed = {contract.security_id: contract for contract in contracts}
            self.contracts = {security_id: self.contracts.get(security_id, {"security_id": security_id, "strike": contract.strike, "option_type": contract.option_type, "expiry": contract.expiry, "bid": [], "ask": []}) for security_id, contract in allowed.items()}
...
def _select_contracts(option_state) -> tuple[list[BankNiftyDepthContract], str | None, float | None]:
    ...
    contracts = [BankNiftyDepthContract(security_id=item[3], strike=item[1], option_type=item[2], expiry=str(expiry)) for item in normalized if item[1] in selected_strikes]
    contracts.sort(key=lambda item: (abs(item.strike - float(underlying_ltp)), item.strike, item.option_type))
    return contracts[:BANKNIFTY_DEPTH_MAX_INSTRUMENTS], str(expiry), float(underlying_ltp)
...
def _parse_depth_message(data: bytes) -> list[tuple[str, str, list[dict]]]:
    ...
    while offset + 12 <= len(data):
        ...
        if response_code in (41, 51) and message_length >= 332:
            ...
            levels = []
            base = offset + 12
            for index in range(BANKNIFTY_DEPTH_LEVELS):
...
class BankNiftyDepthManager:
    """Maintains real-time 20-level premium depth for the nearest 25 BANKNIFTY strikes."""

    def __init__(self, settings, dhan_api, option_manager):
        self.settings = settings
        self.dhan_api = dhan_api
        self.option_manager = option_manager
        self.state = BankNiftyDepthState(settings)
        self.stop_event = threading.Event()
        self.thread: threading.Thread | None = None
        self.ws = None
        self._contracts: list[BankNiftyDepthContract] = []
    ...
    def start(self) -> None:
        ...
        self.thread = threading.Thread(target=self._loop, daemon=True, name="psygrid-banknifty-depth")
        self.thread.start()
    ...
    def _subscribe_payload(self) -> str:
        instruments = [{"ExchangeSegment": BANKNIFTY_DEPTH_EXCHANGE_SEGMENT, "SecurityId": contract.security_id} for contract in self._contracts]
    ...
    def _quote_loop(self) -> None:
        while not self.stop_event.is_set():
            try:
                contracts, expiry, underlying_ltp = _select_contracts(self.option_manager.state)
                if contracts:
                    instruments = [type("DepthInstrument", (), {"security_id": c.security_id, "exchange_segment": BANKNIFTY_DEPTH_EXCHANGE_SEGMENT})() for c in contracts]
            ...
            self.stop_event.wait(BANKNIFTY_DEPTH_QUOTE_REFRESH_SECONDS)

    def _loop(self) -> None:
        quote_thread = threading.Thread(target=self._quote_loop, daemon=True, name="psygrid-banknifty-depth-quotes")
        quote_thread.start()
        while not self.stop_event.is_set():
            try:
                ...
                self.state.set_error(f"{type(exc).__name__}: {exc}")
                self.stop_event.wait(BANKNIFTY_DEPTH_RECONNECT_SECONDS)
        quote_thread.join(timeout=2)


def banknifty_depth_json(state: BankNiftyDepthState) -> dict:
