...
NIFTYPHARMA_SYMBOL = "NIFTY_PHARMA"
NIFTYPHARMA_EXCHANGE_SEGMENT = "IDX_I"
NIFTYPHARMA_INSTRUMENT = "INDEX"
NIFTYPHARMA_MASTER_URL = "https://images.dhan.co/api-data/api-scrip-master.csv"
...
def resolve_niftypharma_security_id() -> str:
    response = requests.get(NIFTYPHARMA_MASTER_URL, timeout=15)
    response.raise_for_status()
    reader = csv.DictReader(io.StringIO(response.text))
    candidates = {_norm("NIFTY PHARMA"), _norm("NIFTY_PHARMA"), _norm("NIFTYPHARMA")}
    matches: list[str] = []
    for row in reader:
        ...
        if str(row.get("SEM_SEGMENT", "")).strip().upper() != "I":
            continue
        if str(row.get("SEM_INSTRUMENT_NAME", "")).strip().upper() != NIFTYPHARMA_INSTRUMENT:
            continue
    ...
    if len(unique) != 1:
        raise RuntimeError(f"Dhan instrument master could not uniquely resolve {NIFTYPHARMA_SYMBOL}: {unique}")
    return unique[0]


class NiftyPharmaInstrument(NiftyInstrument):
    def __init__(self, security_id: str):
        object.__setattr__(self, "security_id", str(security_id))
        object.__setattr__(self, "exchange_segment", NIFTYPHARMA_EXCHANGE_SEGMENT)
        object.__setattr__(self, "instrument", NIFTYPHARMA_INSTRUMENT)


class NiftyPharmaState(NiftyState):
    """RAM-only state for the NIFTY Pharma index endpoint."""


class NiftyPharmaFeed(NiftyFeed):
    def __init__(self, settings, state: NiftyPharmaState, security_id: str):
    ...
    def _on_message(self, _feed, data) -> None:
        ...
        if packet_type == "error":
            self.state.set_feed_status("ERROR", f"Dhan {NIFTYPHARMA_SYMBOL} feed error code={data.get('error_code', data.get('code'))}: {data.get('message', data.get('error_message', 'feed error'))}")
            return
...
class NiftyPharmaManager(NiftyManager):
    def __init__(self, settings, dhan_api):
        self.settings = settings
        self.dhan_api = dhan_api
        security_id = resolve_niftypharma_security_id()
        self.state = NiftyPharmaState(settings)
        self.feed = NiftyPharmaFeed(settings, self.state, security_id)
        self.stop_event = threading.Event()
        self.thread: Optional[threading.Thread] = None
        self.htf_threads: dict[str, threading.Thread] = {}
        self.instrument = NiftyPharmaInstrument(security_id)


def niftypharma_json(state: NiftyPharmaState, security_id: str) -> dict:
    with state.lock:
