...
class RuntimeFreshnessState(PsygridState):
    """Runtime state with packet-receipt freshness and REST fallback snapshots."""
    ...
    def apply_rest_snapshot(self, snapshot: dict) -> None:
        """Store a transparent REST quote fallback without resetting WS freshness."""
        now = time.time()
        with self.lock:
            for security_id, row in snapshot.items():
                ...
                self.rest_fallback_by_security[security_id] = {
                    "ltp": ltp,
                    "timestamp_epoch": now,
                    "source": "DHAN_REST_QUOTE_FALLBACK",
                }
    ...
    def all_live_stale(self, now_epoch: Optional[float] = None) -> bool:
        with self.lock:
            ...
            now_epoch = now_epoch or time.time()
            return all(
                now_epoch - self.last_tick_received_by_security.get(security_id, 0.0) > 30.0
    ...
    def snapshot(self) -> dict:
        with self.lock:
            now_epoch = time.time()
            ages = [max(0.0, now_epoch - value) for value in self.last_tick_received_by_security.values()]
            live_count = sum(1 for age in ages if age <= 30.0)
