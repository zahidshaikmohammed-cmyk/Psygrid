...
SENSEX_OPTIONS_SYMBOL = "SENSEX"
SENSEX_OPTIONS_SECURITY_ID = "51"
SENSEX_OPTIONS_EXCHANGE_SEGMENT = "IDX_I"
SENSEX_OPTIONS_INSTRUMENT = "INDEX"
...
class SensexOptionsInstrument:
    security_id: str = SENSEX_OPTIONS_SECURITY_ID
    exchange_segment: str = SENSEX_OPTIONS_EXCHANGE_SEGMENT
    instrument: str = SENSEX_OPTIONS_INSTRUMENT


class SensexOptionsState:
    """RAM-only state for the isolated SENSEX option-chain domain."""
    ...
    def snapshot(self) -> dict:
        with self.lock:
            ...
            return {
                "service": "PSYGRID",
                "symbol": SENSEX_OPTIONS_SYMBOL,
                "status": self.status,
                "market_status": "OPEN" if market_open else "CLOSED",
                "market_open": market_open,
                "data_source": "DHAN_OPTION_CHAIN_API",
                "security_id": SENSEX_OPTIONS_SECURITY_ID,
                "exchange_segment": SENSEX_OPTIONS_EXCHANGE_SEGMENT,
                "instrument": SENSEX_OPTIONS_INSTRUMENT,
...
def _is_market_open(now: datetime) -> bool:
    """Return regular SENSEX session status in the configured timezone."""
...
class SensexOptionsManager:
    ...
    def start(self) -> None:
        ...
        self.thread = threading.Thread(target=self._loop, daemon=True, name="psygrid-sensex-options")
        self.thread.start()
    ...
    def _load_expiries(self) -> list[str]:
        ...
        if not expiries:
            raise RuntimeError("DHAN_SENSEX_OPTIONS_NO_ACTIVE_EXPIRIES")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if not isinstance(payload, dict):
                    raise RuntimeError("DHAN_SENSEX_OPTIONS_INVALID_RESPONSE")
                rows = _normalize_chain(payload)
                if not rows:
                    raise RuntimeError("DHAN_SENSEX_OPTIONS_EMPTY_CHAIN")
...
def sensex_options_json(state: SensexOptionsState) -> dict:
