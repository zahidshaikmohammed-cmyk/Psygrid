...
NIFTYSMALLCAP100_SYMBOL = "NIFTY_SMALLCAP_100"
NIFTYSMALLCAP100_EXCHANGE_SEGMENT = "IDX_I"
NIFTYSMALLCAP100_INSTRUMENT = "INDEX"
NIFTYSMALLCAP100_MASTER_URL = "https://images.dhan.co/api-data/api-scrip-master.csv"
...
def resolve_niftysmallcap100_security_id() -> str:
    """Resolve the current Dhan security ID from the public instrument master."""
    response = requests.get(NIFTYSMALLCAP100_MASTER_URL, timeout=15)
    response.raise_for_status()
    reader = csv.DictReader(io.StringIO(response.text))

    candidates = {
        _norm("NIFTY SMALLCAP 100"),
        _norm("NIFTY SMLCAP 100"),
        _norm("NIFTY_SMLCAP_100"),
        _norm("NIFTY_SMALLCAP_100"),
        _norm("NIFTYSMALLCAP100"),
        _norm("NIFTYSMLCAP100"),
    }
    matches: list[str] = []
    for row in reader:
        ...
        if exchange != "NSE" or segment != "I" or instrument != NIFTYSMALLCAP100_INSTRUMENT:
            continue
        values = (
    ...
    if len(unique) != 1:
        raise RuntimeError(
            f"Dhan instrument master could not uniquely resolve {NIFTYSMALLCAP100_SYMBOL}: {unique}"
        )
    return unique[0]


class NiftySmallcap100Instrument(NiftyInstrument):
    def __init__(self, security_id: str):
        object.__setattr__(self, "security_id", str(security_id))
        object.__setattr__(self, "exchange_segment", NIFTYSMALLCAP100_EXCHANGE_SEGMENT)
        object.__setattr__(self, "instrument", NIFTYSMALLCAP100_INSTRUMENT)


class NiftySmallcap100State(NiftyState):
    """RAM-only state for the NIFTY Smallcap 100 index endpoint."""


class NiftySmallcap100Feed(NiftyFeed):
    def __init__(self, settings, state: NiftySmallcap100State, security_id: str):
    ...
    def _on_message(self, _feed, data) -> None:
        ...
        if packet_type == "error":
            self.state.set_feed_status(
                "ERROR",
                f"Dhan {NIFTYSMALLCAP100_SYMBOL} feed error code={data.get('error_code', data.get('code'))}: "
...
class NiftySmallcap100Manager(NiftyManager):
    def __init__(self, settings, dhan_api):
        self.settings = settings
        self.dhan_api = dhan_api
        security_id = resolve_niftysmallcap100_security_id()
        self.state = NiftySmallcap100State(settings)
        self.feed = NiftySmallcap100Feed(settings, self.state, security_id)
        self.stop_event = threading.Event()
        self.thread: Optional[threading.Thread] = None
        self.htf_threads: dict[str, threading.Thread] = {}
        self.instrument = NiftySmallcap100Instrument(security_id)


def niftysmallcap100_json(state: NiftySmallcap100State, security_id: str) -> dict:
    with state.lock:
    ...
    return {
        "service": "PSYGRID",
        "schema_version": "3.0",
        "symbol": NIFTYSMALLCAP100_SYMBOL,
        "security_id": str(security_id),
        "exchange_segment": NIFTYSMALLCAP100_EXCHANGE_SEGMENT,
        "instrument": NIFTYSMALLCAP100_INSTRUMENT,
